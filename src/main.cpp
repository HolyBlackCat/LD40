...
namespace Tiles
{
    ...
    #define TILE_LIST \
        ...
        TILE( light           , 255,255, 0 ,255, norm(1)              , 1 , fvec3(1,.8,.6)*400 , 1 , 0 , ) \
...
class Map
{
  ...
  public:
    inline static constexpr int tsz = 24, z_offset = 64, light_z_offset = -20;
...
struct World
{
    ...
    void add_laser(fvec2 pos, float angle, float speed, float max_len, fvec3 color, int index)
    {
        Laser l;
        l.max_len = max_len;
        l.pos = pos;
...
int main(int, char **)
{
    constexpr float player_z = 92, cam_dist = 270/2-50, cam_snap_fac = 0.2, cam_snap_fac_a = 0.4, light_z = -20, light_dist = 25,
                    ...
                    player_trash_rad = 15, trash_rad = 6,
    ...
    auto Tick = [&]
    {
        ...
        { // Lasers
            ...
            while (it != w.laser_list.end())
            {
                ...
                if (w.map.PosSolid(it->pos) && !it->dead)
                {
                    it->dead = 1;
        ...
        { // Trash blocks
            ...
            while (it != w.trash_block_list.end())
            {
                ...
                {
                    if ((it->pos - w.p.pos).len_sqr() <= ipow(player_trash_rad + trash_rad, 2))
                    {
                        destroy = 1;
                        des_quiet = 1;
                        w.p.trash.push_back({{(mi /mul/ it->pos.to_vec3(1)).to_vec2()}, it->vis_angle - w.p.rot});
                    }
                }
        ...
        { // Player
            ...
            if (w.p.dead)
                damp_fac = 20;
            w.p.vel *= 1 - clamp((1 - 0.99) * damp_fac, 0, 1);
            ...
            if (!w.p.dead)
            {
                ...
                w.p.ac += mouse.rel_pos().x * 0.5;
            }
            else
                w.p.hc = w.p.vc = 0;

            w.p.vel += m /mul/ ivec2(w.p.hc,w.p.vc) * 0.1 / mass;
            w.p.ac += fvec2(w.p.hc,0) /cross/ mass_offset + fvec2(0,w.p.vc) /cross/ mass_offset; // Sic!
            w.p.av += w.p.ac / inertia_moment * 0.0002;
            ...
            { // Wall collision check
                if (!w.p.dead)
                {
                    ...
                    {
                        if (w.map.PosSolid(w.p.pos + m /mul/ (it * player_hitbox_off)))
                            w.p.dead = 1;
                    }
                }
                if (!w.p.dead) // Trash wall collisions
                {
                    for (const auto &it : w.p.trash)
                    {
                        if (w.map.PosSolid(w.p.pos + m /mul/ it.offset))
                            w.p.dead = 1;
                    }
                }
            }
    ...
    auto Render = [&]
    {
        ...
        { // Player
            ...
            // Player trash blocks
            {
                ...
                {
                    for (const auto &it : w.p.trash)
                        r.Quad_f(w.p.pos, player_z - 10, ivec2(14)).tex(ivec2(272,215)).center().rotate(w.p.rot).translate(it.offset).rotate(it.vis_angle);
                }
            }

            { // Main light
                w.push_light(fvec3(1,0.8,.6)*200, (w.p.pos + m /mul/ ivec2(0, -light_dist)).to_vec3(light_z), 5);
            }
        ...
        { // Trash blocks
            ...
            {
                r.Quad_f(it.pos, 60, ivec2(14)).tex(ivec2(272,215)).center().rotate(it.vis_angle);
            }
        }
